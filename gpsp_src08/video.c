...
static u16 *screen_vertex = (u16 *)0x441FC100;
...
#define byteswap_soft(variable)                                               \
  variable = (variable >> 16) | (variable << 16);                             \
  variable = ((variable & 0xFF00FF00) >> 8) |                                 \
   ((variable & 0x00FF00FF) << 8)                                             \

#define byteswap(variable)                                                    \
  asm("bswap %0" : "=r"(variable) : "0"(variable))                            \
...
{                                                                             \
  ...
  {                                                                           \
    ...
    middle_x -= x_delta;                                                      \
    advance_dest_ptr_##combine_op(x_delta);                                   \
    obj_width -= x_delta;                                                     \
                                                                              \
    if(obj_width <= 0)                                                        \
      continue;                                                               \
...
void init_video()
{
  ...
  screen_vertex[3] = 0;
  screen_vertex[4] = 0;
  screen_vertex[5] = GBA_SCREEN_WIDTH;
  screen_vertex[6] = GBA_SCREEN_HEIGHT;
...
void init_video()
{
  ...
  screen = SDL_SetVideoMode(240, 160, 16, 0);
  SDL_ShowCursor(0);
}
...
void set_gba_resolution(video_scale_type scale)
{
  ...
  switch(scale)
  {
    case unscaled:
      screen_vertex[2] = 120;
      screen_vertex[3] = 56;
      screen_vertex[7] = GBA_SCREEN_WIDTH + 120;
      screen_vertex[8] = GBA_SCREEN_HEIGHT + 56;
      break;

    case scaled_aspect:
      screen_vertex[2] = 36;
      screen_vertex[3] = 0;
      screen_vertex[7] = 408 + 36;
      screen_vertex[8] = PSP_SCREEN_HEIGHT;
      break;
...
void video_resolution_small()
{
  if(current_scale != screen_scale)
  {
    ...
    screen = SDL_SetVideoMode(small_resolution_width,
     small_resolution_height, 16, 0);
...
void set_gba_resolution(video_scale_type scale)
{
  ...
  {
    printf("setting screen scale to %d\n", scale);
    screen_scale = scale;
    switch(scale)
    {
      case unscaled:
