...
#define block_lookup_translate_arm(mem_type, smc_ptr)                         \
  translation_result = translate_block_arm(pc, mem_type##_translation_region, \
   (u8 *)smc_ptr)                                                             \

#define block_lookup_translate_thumb(mem_type, smc_ptr)                       \
  translation_result = translate_block_thumb(pc,                              \
   mem_type##_translation_region, (u8 *)smc_ptr)                              \

#define block_lookup_translate_dual(mem_type, smc_ptr)                        \
  if(thumb)                                                                   \
  {                                                                           \
    translation_result = translate_block_thumb(pc,                            \
     mem_type##_translation_region, (u8 *)smc_ptr);                           \
...
#define block_lookup_translate(instruction_type, mem_type, smc_ptr)           \
  block_tag = *location;                                                      \
  ...
  {                                                                           \
    ...
    mem_type##_block_tag_top++;                                               \
                                                                              \
    block_lookup_translate_##instruction_type(mem_type, smc_ptr);             \
    translation_recursion_level--;                                            \
                                                                              \
...
{                                                                             \
  ...
  {                                                                           \
    case 0x0:                                                                 \
      location = (u16 *)(bios_rom + (pc & 0x3FFF) + 0x4000);                  \
      block_lookup_translate(type, bios, NULL);                               \
      break;                                                                  \
                                                                              \
    case 0x2:                                                                 \
      location = (u16 *)(ewram + (pc & 0x7FFF) + ((pc & 0x38000) * 2));       \
      block_lookup_translate(type, ram, location);                            \
      break;                                                                  \
                                                                              \
    case 0x3:                                                                 \
    ...
    {                                                                         \
      ...
      {                                                                       \
        ...
        *block_ptr_address = (u32 *)rom_translation_ptr;                      \
        ...
        block_lookup_translate_##type(rom, NULL);                             \
        translation_recursion_level--;                                        \
                                                                              \
        /* If the translation failed then pass that failure on if we're in    \
         a recursive level, or try again if we've hit the bottom. */          \
...
#define arm_load_opcode()                                                     \
  ...
  condition = opcode >> 28;                                                   \
                                                                              \
  /* Hack that will hopefully capture blocks that are being jumped to but     \
...
#define arm_set_condition(_condition)                                         \
  if((opcode & 0xE100060) == 0x0100000)                                       \
    block_data[block_data_position].condition = _condition | 0x20;            \
  else                                                                        \
...
#define arm_base_cycles()                                                     \
  cycles_per_instruction = waitstate_cycles_sequential[pc >> 24][2]           \
...
#define thumb_base_cycles()                                                   \
  cycles_per_instruction = waitstate_cycles_sequential[pc >> 24][1]           \
...
#define smc_write_arm_yes()                                                   \
  if(*((u32 *)smc_ptr) == 0x00000000)                                         \
  {                                                                           \
    *((u32 *)smc_ptr) = 0xFFFFFFFF;                                           \
  }                                                                           \
  smc_ptr += 4                                                                \

#define smc_write_thumb_yes()                                                 \
  if(*((u16 *)smc_ptr) == 0x0000)                                             \
  {                                                                           \
    *((u16 *)smc_ptr) = 0xFFFF;                                               \
  }                                                                           \
  smc_ptr += 2                                                                \
