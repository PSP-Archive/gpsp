...
#define branch_target_counter()                                               \
{                                                                             \
  u16 *location;                                                              \
  switch(pc >> 24)                                                            \
  {                                                                           \
    case 0x0:                                                                 \
      location = (u16 *)(bios_rom + (pc & 0x1FFF) + 0x4000);                  \
      break;                                                                  \
                                                                              \
    case 0x2:                                                                 \
      location = (u16 *)(ewram + (pc & 0x3FFF) + ((pc & 0x38000) * 2));       \
      break;                                                                  \
                                                                              \
    case 0x3:                                                                 \
      location = (u16 *)(iwram + (pc & 0x3FFF));                              \
      break;                                                                  \
                                                                              \
    case 0x8 ... 0xD:                                                         \
      location = (u16 *)(gamepak_rom_branch_targets + (pc & 0x1FFFFFF));      \
      break;                                                                  \
  }                                                                           \
                                                                              \
  if((*location) < BRANCH_ACTIVITY_THRESHOLD)                                 \
  {                                                                           \
    (*location)++;                                                            \
    if((*location) == 1)                                                      \
      branch_targets++;                                                       \
    if((*location) == BRANCH_ACTIVITY_THRESHOLD)                              \
      high_frequency_branch_targets++;                                        \
  }                                                                           \
}                                                                             \

#define arm_update_pc()                                                       \
  pc = reg[REG_PC];                                                           \
  /*branch_target_counter();*/                                                \
...
{                                                                             \
  ...
  {                                                                           \
    *((type *)((u8 *)map + (address & 0x7FFF))) = value;                      \
  }                                                                           \
  else                                                                        \
  {                                                                           \
    if(write_memory##size(address, value))                                    \
      goto alert;                                                             \
  }                                                                           \
}                                                                             \
...
{                                                                             \
  ...
  {                                                                           \
    if(write_memory32(address, value))                                        \
      goto alert;                                                             \
  }                                                                           \
}                                                                             \
...
#define arm_block_address_preadjust_up_full()                                 \
  address += word_bit_count(reg_list) * 4                                     \
...
#define arm_block_address_preadjust_down()                                    \
  address -= (word_bit_count(reg_list) * 4) - 4                               \
...
#define arm_block_access_pc_preadjust_load()                                  \
  arm_pc_offset(4)                                                            \

#define arm_block_access_pc_preadjust_store()                                 \
  arm_pc_offset(8)                                                            \

#define arm_block_access_pc_postadjust_load()                                 \
  arm_update_pc()                                                             \

#define arm_block_access_pc_postadjust_store()                                \
  arm_pc_offset(-4)                                                           \
...
#define arm_block_memory_map_load  memory_map_read
#define arm_block_memory_map_store memory_map_write
...
{                                                                             \
  ...
  u32 address = reg[rn] & ~0x03;                                              \
  u8 *address_region;                                                         \
  arm_block_address_preadjust_##pre_op();                                     \
  arm_block_access_pc_preadjust_##access_type();                              \
                                                                              \
  sprint_##s_bit(access_type, pre_op, post_op, wb);                           \
                                                                              \
  if(!((address ^ (address + (word_bit_count(reg_list) * 4))) >> 15) &&       \
   (address_region = arm_block_memory_map_##access_type[address >> 15]))      \
  {                                                                           \
    for(i = 0; i < 16; i++)                                                   \
    {                                                                         \
      if((reg_list >> i) & 0x01)                                              \
      {                                                                       \
        access_type##_block_memory(address, reg[i]);                          \
        offset += 4;                                                          \
      }                                                                       \
    }                                                                         \
  }                                                                           \
  ...
  arm_block_access_pc_postadjust_##access_type();                             \
}                                                                             \
...
#define execute_arm_instruction()                                             \
  ...
  {                                                                           \
    ...
    case 0x80:                                                                \
      /* STMDA rn, rlist */                                                   \
      arm_block_memory(store, down, no, no, no);                              \
      break;                                                                  \
                                                                              \
    case 0x81:                                                                \
      /* LDMDA rn, rlist */                                                   \
      arm_block_memory(load, down, no, no, no);                               \
      break;                                                                  \
                                                                              \
    case 0x82:                                                                \
      /* STMDA rn!, rlist */                                                  \
      arm_block_memory(store, no, down, yes, no);                             \
      break;                                                                  \
                                                                              \
    case 0x83:                                                                \
      /* LDMDA rn!, rlist */                                                  \
      arm_block_memory(load, no, down, yes, no);                              \
      break;                                                                  \
                                                                              \
    case 0x84:                                                                \
      /* STMDA rn, rlist^ */                                                  \
      arm_block_memory(store, down, no, no, yes);                             \
      break;                                                                  \
                                                                              \
    case 0x85:                                                                \
      /* LDMDA rn, rlist^ */                                                  \
      arm_block_memory(load, down, no, no, yes);                              \
      break;                                                                  \
                                                                              \
    case 0x86:                                                                \
      /* STMDA rn!, rlist^ */                                                 \
      arm_block_memory(store, no, down, yes, yes);                            \
      break;                                                                  \
                                                                              \
    case 0x87:                                                                \
      /* LDMDA rn!, rlist^ */                                                 \
      arm_block_memory(load, no, down, yes, yes);                             \
      break;                                                                  \
                                                                              \
    case 0x88:                                                                \
      /* STMIA rn, rlist */                                                   \
      arm_block_memory(store, no, no, no, no);                                \
      break;                                                                  \
                                                                              \
    case 0x89:                                                                \
      /* LDMIA rn, rlist */                                                   \
      arm_block_memory(load, no, no, no, no);                                 \
      break;                                                                  \
                                                                              \
    case 0x8A:                                                                \
      /* STMIA rn!, rlist */                                                  \
      arm_block_memory(store, no, up, yes, no);                               \
      break;                                                                  \
                                                                              \
    case 0x8B:                                                                \
      /* LDMIA rn!, rlist */                                                  \
      arm_block_memory(load, no, up, yes, no);                                \
      break;                                                                  \
                                                                              \
    case 0x8C:                                                                \
      /* STMIA rn, rlist^ */                                                  \
      arm_block_memory(store, no, no, no, yes);                               \
      break;                                                                  \
                                                                              \
    case 0x8D:                                                                \
      /* LDMIA rn, rlist^ */                                                  \
      arm_block_memory(load, no, no, no, yes);                                \
      break;                                                                  \
                                                                              \
    case 0x8E:                                                                \
      /* STMIA rn!, rlist^ */                                                 \
      arm_block_memory(store, no, up, yes, yes);                              \
      break;                                                                  \
                                                                              \
    case 0x8F:                                                                \
      /* LDMIA rn!, rlist^ */                                                 \
      arm_block_memory(load, no, up, yes, yes);                               \
      break;                                                                  \
                                                                              \
    case 0x90:                                                                \
      /* STMDB rn, rlist */                                                   \
      arm_block_memory(store, down_full, no, no, no);                         \
      break;                                                                  \
                                                                              \
    case 0x91:                                                                \
      /* LDMDB rn, rlist */                                                   \
      arm_block_memory(load, down_full, no, no, no);                          \
      break;                                                                  \
                                                                              \
    case 0x92:                                                                \
      /* STMDB rn!, rlist */                                                  \
      arm_block_memory(store, down_full, no, yes, no);                        \
      break;                                                                  \
                                                                              \
    case 0x93:                                                                \
      /* LDMDB rn!, rlist */                                                  \
      arm_block_memory(load, down_full, no, yes, no);                         \
      break;                                                                  \
                                                                              \
    case 0x94:                                                                \
      /* STMDB rn, rlist^ */                                                  \
      arm_block_memory(store, down_full, no, no, yes);                        \
      break;                                                                  \
                                                                              \
    case 0x95:                                                                \
      /* LDMDB rn, rlist^ */                                                  \
      arm_block_memory(load, down_full, no, no, yes);                         \
      break;                                                                  \
                                                                              \
    case 0x96:                                                                \
      /* STMDB rn!, rlist^ */                                                 \
      arm_block_memory(store, down_full, no, yes, yes);                       \
      break;                                                                  \
                                                                              \
    case 0x97:                                                                \
      /* LDMDB rn!, rlist^ */                                                 \
      arm_block_memory(load, down_full, no, yes, yes);                        \
      break;                                                                  \
                                                                              \
    case 0x98:                                                                \
      /* STMIB rn, rlist */                                                   \
      arm_block_memory(store, up, no, no, no);                                \
      break;                                                                  \
                                                                              \
    case 0x99:                                                                \
      /* LDMIB rn, rlist */                                                   \
      arm_block_memory(load, up, no, no, no);                                 \
      break;                                                                  \
                                                                              \
    case 0x9A:                                                                \
      /* STMIB rn!, rlist */                                                  \
      arm_block_memory(store, up_full, no, yes, no);                          \
      break;                                                                  \
                                                                              \
    case 0x9B:                                                                \
      /* LDMIB rn!, rlist */                                                  \
      arm_block_memory(load, up_full, no, yes, no);                           \
      break;                                                                  \
                                                                              \
    case 0x9C:                                                                \
      /* STMIB rn, rlist^ */                                                  \
      arm_block_memory(store, up, no, no, yes);                               \
      break;                                                                  \
                                                                              \
    case 0x9D:                                                                \
      /* LDMIB rn, rlist^ */                                                  \
      arm_block_memory(load, up, no, no, yes);                                \
      break;                                                                  \
                                                                              \
    case 0x9E:                                                                \
      /* STMIB rn!, rlist^ */                                                 \
      arm_block_memory(store, up_full, no, yes, yes);                         \
      break;                                                                  \
                                                                              \
    case 0x9F:                                                                \
      /* LDMIB rn!, rlist^ */                                                 \
      arm_block_memory(load, up_full, no, yes, yes);                          \
      break;                                                                  \
                                                                              \
    case 0xA0:                                                                \
    case 0xA1:                                                                \
...
void print_arm_debug(cycles)
{
  ...
  print_flags();
  print_stack();
  printf("vcount: %d\t\tinstructions in: %d\n", io_registers[REG_VCOUNT],
   instruction_count);
  printf("cycles remaining: %d\t\tROM cache: %d\n", cycles,
...
*/      
    ...
    switch(key)
    {
      ...
      case 'c':
        printf("break after N instructions: ");
