...
#define MAX_GAMEPAK_RAM_BUFFER (16 * 1024 * 1024)
#define GAMEPAK_RAM_PAGES (MAX_GAMEPAK_RAM_BUFFER / (32 * 1024))

// Enough to map the gamepak RAM space.
gamepak_swap_entry_type gamepak_memory_map[GAMEPAK_RAM_PAGES];
...
backup_type_type backup_type = BACKUP_SRAM;
...
#define read_backup8()                                                        \
  ...
  {                                                                           \
    value = gamepak_backup[address & 0xFFFF];                                 \
  }                                                                           \
  else                                                                        \
                                                                              \
  ...
  {                                                                           \
    value = flash_bank_ptr[address & 0xFFFF];                                 \
  }                                                                           \
  ...
  {                                                                           \
    ...
    default:                                                                  \
      if(reg[REG_CPSR] & 0x20)                                                \
        value = (reg[REG_PC] & 0xFFFF) | (reg[REG_PC] >> 16);                 \
      else                                                                    \
        value = reg[REG_PC];                                                  \
      break;                                                                  \
  }                                                                           \
  ...
  {                                                                           \
    ...
    {                                                                         \
      ...
      dma[dma_number].source_address =                                        \
       address32(io_registers, (dma_number * 12) + 0xB0);                     \
...
#define write_backup8()                                                       \
  ...
  if((address == 0xE005555) && (flash_mode != FLASH_WRITE_MODE))              \
  {                                                                           \
    ...
    {                                                                         \
      ...
      flash_command_position = 0;                                             \
                                                                              ...
                                                                              \
  if((address == 0xE002AAA) && (value == 0x55) &&                             \
   (flash_command_position == 1))                                             \
  {                                                                           \
  ...
  {                                                                           \
    ...
    if((flash_command_position == 0) &&                                       \
     (flash_mode == FLASH_BANKSWITCH_MODE) && (address == 0xE000000))         \
    {                                                                         \
    ...
    {                                                                         \
      ...
      backup_update = write_backup_delay;                                     \
      flash_bank_ptr[address & 0xFFFF] = value;                               \
      flash_mode = FLASH_BASE_MODE;                                           \
    }                                                                         \
    else                                                                      \
                                                                              \
      ...
      {                                                                       \
        ...
        case EEPROM_WRITE_ADDRESS_MODE:                                       \
          ...
          {                                                                   \
            ...
            {                                                                 \
              eeprom_address = ((eeprom_buffer[1] >> 2) |                     \
               (eeprom_buffer[0] << 8)) * 8;                                  \
            }                                                                 \
            address16(eeprom_buffer, 0) = 0;                                  \
            eeprom_counter = 0;                                               \
...
u16 function_cc read_memory16(u32 address)
{
  u16 value;
  if(address & 0x01)
  {
       ...
       != -1)
      {
        ...
        while(fgets(current_line, 256, config_file))
        {
           ...
           != -1)
          {
            ...
            {
              if(!strcmp(current_value, "yes"))
                iwram_stack_optimize = 1;

              if(!strcmp(current_value, "no"))
                iwram_stack_optimize = 0;
            }

            if(!strcmp(current_variable, "flash_rom_type"))
            {
              if(!strcmp(current_value, "128KB"))
              {
                printf("setting 128KB flash\n");
                flash_device_id = FLASH_DEVICE_MACRONIX_128KB;
              }

              if(!strcmp(current_value, "64KB"))
                flash_device_id = FLASH_DEVICE_MACRONIX_64KB;
            }
          }
        }
...
s32 load_gamepak_raw(char *name)
{
  ...
  if(file_check_valid(gamepak_file))
  {
    ...
    if(gamepak_size <= MAX_GAMEPAK_RAM_BUFFER)
    {
...
/*    if(!strcmp(dot_position, ".mb"))
    {
      u32 i;
      printf("loading multiboot ROM (size %d)\n", file_size);
      for(i = 0; (i < (gamepak_size / 0x8000)) && (i < 8); i++)
      {
        printf("copying to ewram page %x\n", i);
        memcpy(ewram + (i * 0x10000) + 0x8000,
         gamepak_rom + (i * 0x8000), 0x8000);
        printf("%x\n", read_memory32(0x2000260));
      }
    } */
...
void init_memory_gamepak()
{
  ...
  if(gamepak_size > MAX_GAMEPAK_RAM_BUFFER)
  {
    // Large (> 16MB) ROMs get special treatment because they
...
void init_memory()
{
  ...
  map_region(read, 0x0000000, 0x2000000, 1, bios_rom);
  ...
  // For some reason mapping VRAM makes things super slow in games
  // where you'd expect it to benefit.. humm
  // That aside, this breaks 8bit write compatability, but I don't
  // think anything important relies on it (just some homebrew)
  //map_vram(write);
...
void save_state(char *savestate_filename, u16 *screen_capture)
{
  ...
  {
    time_t current_time;
    file_write(savestate_file, screen_capture, 240 * 160 * 2);
