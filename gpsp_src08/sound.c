...
#define gbc_sound_render_sample_right()                                       \
  sound_buffer[buffer_index] += (current_sample * volume_right) >> 22         \

#define gbc_sound_render_sample_left()                                        \
  sound_buffer[buffer_index + 1] += (current_sample * volume_left) >> 22      \
...
#define sound_copy(source_offset, length)                                     \
  _length = (length) / 2;                                                     \
...
void sound_callback(void *userdata, Uint8 *stream, int length)
{
  ...
  if((sound_buffer_base + sample_length) >= BUFFER_SIZE)
  {
    ...
    sound_copy(sound_buffer_base, partial_length);
    sound_copy(0, length - partial_length);
    sound_buffer_base = (length - partial_length) / 2;
  }
  else
  {
    sound_copy(sound_buffer_base, length);
...
{
  gbc_sound_buffer_index =
   (sound_buffer_base + SOUND_BUFFER_SIZE) % BUFFER_SIZE;
  SDL_PauseAudio(1);
  SDL_CondSignal(sound_cv);
}
